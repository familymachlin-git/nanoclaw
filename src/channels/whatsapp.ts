...
async function resolveWaWebVersion(): Promise<[number, number, number]> {
  ...
  try {
    const { version } = await fetchLatestWaWebVersion({ signal: AbortSignal.timeout(10000) }); // wa-watchdog: never hang
    if (version) {
...
// wa-watchdog: tunables (ms), env-overridable for testing
const WA_RECONNECT_MAX_DELAY_MS = Number(process.env.WA_RECONNECT_MAX_DELAY_MS || 300000);
const WA_WATCHDOG_EXIT_MS = Number(process.env.WA_WATCHDOG_EXIT_MS || 300000);
const WA_WATCHDOG_POLL_MS = Number(process.env.WA_WATCHDOG_POLL_MS || 60000);
...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    // wa-watchdog: self-heal for stuck reconnects (managed by family-brain
    // tools/apply-nanoclaw-wa-watchdog.sh — do not edit by hand).
    // Layer 1: reconnectWithBackoff retries connectSocket until it succeeds.
    // Layer 2: if disconnected > WA_WATCHDOG_EXIT_MS after having been connected
    // once, exit(1) so systemd (Restart=always) performs a clean cold start.
    let waReconnecting = false;
    let waWatchdogArmed = false;
    let waDisconnectedSince = 0;
    async function reconnectWithBackoff() {
        if (waReconnecting) return;
        waReconnecting = true;
        let waDelay = RECONNECT_DELAY_MS;
        try {
            while (!shuttingDown) {
                try {
                    await connectSocket();
                    ...
                    log.error('Reconnect attempt failed, retrying with backoff', { err, nextRetryMs: waDelay });
                    await new Promise((r) => setTimeout(r, waDelay));
                    waDelay = Math.min(waDelay * 2, WA_RECONNECT_MAX_DELAY_MS);
                }
            }
        } finally {
    ...
    setInterval(() => {
        if (connected) {
            waWatchdogArmed = true;
            waDisconnectedSince = 0;
            return;
        }
        if (!waWatchdogArmed || shuttingDown) return;
        if (waDisconnectedSince === 0) {
            waDisconnectedSince = Date.now();
            return;
        }
        const waDownMs = Date.now() - waDisconnectedSince;
        if (waDownMs > WA_WATCHDOG_EXIT_MS) {
            log.error('WhatsApp disconnected past watchdog limit — exiting so systemd restarts the process', { downMs: waDownMs });
            process.exit(1);
        }
    }, WA_WATCHDOG_POLL_MS).unref();
      ...
      sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
          ...
          if (shouldReconnect) {
                        ...
                        // wa-watchdog: retry forever with backoff (stock code gave up after 2 attempts)
