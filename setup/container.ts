...
export async function run(args: string[]): Promise<void> {
  ...
  const existingImage = spawnSync(runCmd, ['image', 'inspect', image], { stdio: 'ignore' });
  ...
  let buildOk = existingImage.status === 0;
  if (buildOk) {
    log.info('Container image already exists; skipping build', { image });
  } else {
